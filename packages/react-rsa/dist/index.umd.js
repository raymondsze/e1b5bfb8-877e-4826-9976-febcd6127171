(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('prop-types'), require('react'), require('react-redux'), require('reselect'), require('redux-actions'), require('react-apollo'), require('apollo-client'), require('history/createBrowserHistory'), require('react-intl'), require('immutable'), require('react-router-redux'), require('react-router'), require('redux'), require('redux-saga'), require('redux-immutable')) :
  typeof define === 'function' && define.amd ? define(['exports', 'prop-types', 'react', 'react-redux', 'reselect', 'redux-actions', 'react-apollo', 'apollo-client', 'history/createBrowserHistory', 'react-intl', 'immutable', 'react-router-redux', 'react-router', 'redux', 'redux-saga', 'redux-immutable'], factory) :
  (factory((global.react-rsa = global.react-rsa || {}),global.PropTypes,global.React,global.reactRedux,global.reselect,global.reduxActions,global.reactApollo,global.ApolloClient,global.createBrowserHistory,global.reactIntl,global.immutable,global.reactRouterRedux,global.reactRouter,global.redux,global.createSagaMiddleware,global.reduxImmutable));
}(this, (function (exports,PropTypes,React,reactRedux,reselect,reduxActions,reactApollo,ApolloClient,createBrowserHistory,reactIntl,immutable,reactRouterRedux,reactRouter,redux,createSagaMiddleware,reduxImmutable) { 'use strict';

PropTypes = 'default' in PropTypes ? PropTypes['default'] : PropTypes;
var React__default = 'default' in React ? React['default'] : React;
var ApolloClient__default = 'default' in ApolloClient ? ApolloClient['default'] : ApolloClient;
createBrowserHistory = 'default' in createBrowserHistory ? createBrowserHistory['default'] : createBrowserHistory;
createSagaMiddleware = 'default' in createSagaMiddleware ? createSagaMiddleware['default'] : createSagaMiddleware;
...
var _createActions = reduxActions.createActions(defineProperty({}, LOCALE_CHANGE, function (locale) {
  return {
    locale: locale
  };
}));
...
var getLocale = reselect.createSelector(getLanguage, function (languageState) {
  return languageState.get('locale');
});

var getLocaleMessages = reselect.createSelector(getLanguage, function (languageState) {
  return languageState.get('messages').toJS();
});

var mapStateToProps = reselect.createSelector(getLocale, getLocaleMessages, function (locale, messages) {
...
function withLocale(Component$$1) {
  return reactRedux.connect(mapStateToProps, mapDispatchToProps)(Component$$1);
}
...
var reducers = {
  language: reduxActions.handleActions(defineProperty({}, LOCALE_CHANGE, function (state, _ref) {
...
var mapStateToProps$1 = reselect.createSelector(getLocale, getLocaleMessages, function (locale, messages) {
  return { locale: locale, messages: messages };
});

var withReducers$1 = withReducers(reducers);
var withRedux = reactRedux.connect(mapStateToProps$1);
...
var ScrollTop$1 = reactRouter.withRouter(ScrollTop);
...
var routeInitialState = immutable.fromJS({
...
function createReducer(asyncReducers, apolloClient) {
  return reduxImmutable.combineReducers(_extends({
    route: routeReducer,
...
function configureStore(_ref) {
  ...
  var middlewares = [sagaMiddleware, reactRouterRedux.routerMiddleware(history), client.middleware()];

  var finalEnhancers = [redux.applyMiddleware.apply(undefined, middlewares)].concat(toConsumableArray(enhancers));

  // If Redux DevTools Extension is installed use it, otherwise use Redux compose
  /* eslint-disable no-underscore-dangle */
  var composeEnhancers =
  // if client side and not in production, enable the redux dev tool
  process.env.NODE_ENV !== 'production' && (typeof window === 'undefined' ? 'undefined' : _typeof(window)) === 'object' && window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__ ? window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__ : redux.compose;
  /* eslint-enable */

  var store = redux.createStore(createReducer(reducers, client), immutable.fromJS(state), composeEnhancers.apply(undefined, toConsumableArray(finalEnhancers)));
...
var RSAProvider = (_temp = _class = function (_Component) {
  ...
  createClass(RSAProvider, [{
    ...
    get: function get() {
      ...
      return new ApolloClient__default(client);
    }
  }, {
    key: 'store',
    get: function get() {
  ...
  state: {},
  client: {
    networkInterface: ApolloClient.createNetworkInterface({
      uri: '/graphql',
      opts: {
...
exports.injectReducers = withReducers;
exports.injectSagas = withSagas;
